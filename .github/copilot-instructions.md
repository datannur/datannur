...
- **`app/` is the build output**: NEVER modify files in `/app/` directly — it is entirely generated by `npm run build` (Vite copies `/public/` and bundles `/src/`). All source files live in `/src/` and `/public/`
