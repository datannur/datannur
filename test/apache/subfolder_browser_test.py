...
def assert_generated_html_is_portable() -> None:
    datasets_html = DIST_DIR / "data" / "static" / "datasets.html"
    tag_html = DIST_DIR / "data" / "static" / "tag" / "anonymous_data.html"
    ...
    with tempfile.TemporaryDirectory(prefix="datannur-apache-empty-root-") as tmp:
        ...
        try:
            ...
            with sync_playwright() as playwright:
                ...
                assert_page(
                    page,
                    f"{BASE_URL}/tag/anonymous_data",
                    "Mot clé | Données anonymisées",
                    "tag",
                )
                page.reload(wait_until="domcontentloaded")
                page.wait_for_selector('body[page="tag"]')

                home_link = page.locator('a.navbar-item[href="/datannur/"]').first
                if home_link.get_attribute("href") != "/datannur/":
                ...
                assert_spa_fallback_page(
                    page,
                    f"{BASE_URL}/variable/usage_internet__connexion_domicile",
                    "Variable | Connexion à domicile",
                    "variable",
                )
                ...
                page.wait_for_selector('a.navbar-item[href="/datannur/"]')
                page.locator('a.navbar-item[href="/datannur/"]').first.click()
                page.wait_for_function(
                    """expectedUrl =>
                ...
                tag_link = page.locator('a[href="/datannur/tag/anonymous_data"]').first
                if tag_link.get_attribute("href") != "/datannur/tag/anonymous_data":
                ...
                tag_link.click()
                page.wait_for_url(f"{BASE_URL}/tag/anonymous_data")
                page.reload(wait_until="domcontentloaded")
                page.wait_for_selector('body[page="tag"]')
                if page.title() != "Mot clé | Données anonymisées":
                    sys.exit(
                ...
                page.goto(
                    f"{BASE_URL}/about?tab=aboutStructure",
                    wait_until="domcontentloaded",
                )
                page.wait_for_selector(".mermaid-block svg a")
                mermaid_hrefs = page.locator(".mermaid-block svg a").evaluate_all(
