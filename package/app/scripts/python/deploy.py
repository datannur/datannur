...
CONFIG_PATH = DATA_DIR / "deploy.config.json"
...
def load_config(config_path: Path) -> DeployConfig:
    if not config_path.exists():
        sys.exit(
            ...
            "Create data/deploy.config.json from app/data-template/deploy.config.json."
        )
...
def main():
    ...
    try:
        subprocess.run(build_rsync_command(config), cwd=PACKAGE_DIR, check=True)
