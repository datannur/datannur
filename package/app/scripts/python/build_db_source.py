...
from _local_runtime import DATA_DIR, PACKAGE_DIR

SOURCE_DIR = DATA_DIR / "db-source"
...
def read_json_table(path: Path) -> list[dict[str, Any]]:
    try:
        ...
        fail(f"{path.relative_to(PACKAGE_DIR)} is not valid JSON: {error}")
    except UnicodeDecodeError:
        fail(f"{path.relative_to(PACKAGE_DIR)} is not valid UTF-8")
    except OSError as error:
        fail(f"Cannot read {path.relative_to(PACKAGE_DIR)}: {error}")

    if not isinstance(data, list):
        fail(f"{path.relative_to(PACKAGE_DIR)} must contain a JSON array")

    for index, item in enumerate(data):
        if not isinstance(item, dict):
            fail(f"{path.relative_to(PACKAGE_DIR)} item {index} must be an object")
...
def compile_markdown_docs() -> set[str]:
    ...
    for source_path in sorted(MD_SOURCE_DIR.glob("*.md")):
        try:
            ...
            fail(f"{source_path.relative_to(PACKAGE_DIR)} is not valid UTF-8")
        except OSError as error:
            fail(f"Cannot read {source_path.relative_to(PACKAGE_DIR)}: {error}")
...
def compile_csv_previews() -> set[str]:
    ...
    for source_path in sorted(DATASET_SOURCE_DIR.glob("*.csv")):
        try:
            ...
            fail(f"Cannot parse {source_path.relative_to(PACKAGE_DIR)}: {error}")
        except OSError as error:
            fail(f"Cannot read {source_path.relative_to(PACKAGE_DIR)}: {error}")
...
def main() -> None:
    if not SOURCE_DIR.exists():
        fail(f"Source directory not found: {SOURCE_DIR.relative_to(PACKAGE_DIR)}")

    if not has_source_files():
        fail(
            "No JSON, Markdown, or CSV source files found in "
            f"{SOURCE_DIR.relative_to(PACKAGE_DIR)}"
        )
