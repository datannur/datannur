...
# PowerShell 5.1+, zero dependencies. Serves files from the app root
...
Write-Host "serve-app: listening on http://localhost:$Port/ (root: $AppDir)"
...
while ($listener.IsListening) {
    ...
    try {
        ...
        $fullPath = [IO.Path]::GetFullPath((Join-Path $AppDir $relPath))

        # Path traversal check: require exact match OR prefix ending with separator
        $appRootNorm = [IO.Path]::GetFullPath($AppDir).TrimEnd([IO.Path]::DirectorySeparatorChar)
        ...
        $useGzip = $false
        if ($accept -and $accept -match 'gzip' -and $entry.Bytes.Length -gt 1024) {
