...
In corporate/institutional contexts (Windows, shared network drive, no admin rights), datannur can be installed at user level. Scripts live in `app/scripts/windows/` inside the distributed app. Each target is independent and has its own `install`/`uninstall` pair (except the shortcut, which uninstalls via the Start Menu).
...
- `install-llm.bat` — installs a launcher for the local LLM proxy that auto-starts on `http://localhost:<llmProxyPort>` (default `61292`).
...
On Linux, an additional `python3 datannur.py install-shortcut` creates a `~/.local/share/applications/datannur.desktop` entry that opens `index.html` in a chromeless browser window (`file://` mode, no server). macOS is not covered by `install-shortcut`: for a native Dock app, use `python3 datannur.py install-autostart -- app` then open the `http://localhost` URL in Safari and use **File > Add to Dock**.
